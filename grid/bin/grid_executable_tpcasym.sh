...
cp -r ${filesFromSender}/run_tpc_asym_study.C .
...
echo "@@ run"
root -l -b -q "run_tpc_asym_study.C(\"input_list_${nProcess}.txt\", \"${nProcess}\", true)" &> log_${nProcess}.log
ls -alh
...
outFILE=${thisOutputCreationDir}/output/root/output_tpc_asym_${nProcess}.root
if [ -f "$outFILE" ]; then
  echo "ifdh cp ${thisOutputCreationDir}/output/root/output_tpc_asym_${nProcess}.root ${outDir}/${DFPREFIX}_${nProcess}.root"
  ifdh cp ${thisOutputCreationDir}/output/root/output_tpc_asym_${nProcess}.root ${outDir}/${DFPREFIX}_${nProcess}.root
