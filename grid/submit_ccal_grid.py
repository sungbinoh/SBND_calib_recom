...
def run_grid(inputfiles):
    ...
    OutputDir = SBNDCALIB_GRID_OUT_DIR + "/ccal/" + args.output + "__" + timestamp
    ...
    os.system('cp ./grid/bin/grid_executable_ccal.sh %s' %MasterJobDir)
    ...
    cp_lifetime_loop = "cp " + CALIB_WORKING_DIR + "/src/run_recom_loop_emb.C " + MasterJobDir
...
"file://$(pwd)/grid_executable_ccal.sh" \\
"%s" \\
