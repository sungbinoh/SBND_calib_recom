...
void draw_rr_vs_dedx_each_angle(TString angle_bin, TString angle_str){
  ...
  TString filename = input_file_dir + "/output_dedx_loop_emb_run_" + run_str + ".root";
  ...
  TString outfile_str = output_plot_dir + "/mc/emb/rr_dedx/rr_vs_dedx" + angle_bin + "." + image_type;
  if(isdata) outfile_str = output_plot_dir + "/run_" + run_str + "/emb/rr_dedx/plane" + plane_str + "_rr_vs_dedx" + angle_bin + "." + image_type;
...
void draw_rr_vs_dedx_emb(int this_run_num = 0, int this_rebin_x = 1, int this_rebin_y = 1, int this_plane = 2){
  ...
  for(int i = 0; i < N_angle_bins; i++){
    draw_rr_vs_dedx_each_angle(angle_bins[i], angle_strs[i]);
  }
}
